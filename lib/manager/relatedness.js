 ...
 */
import { extractPaths, familyOfPath } from "./drift.js";
export const DEFAULT_RELATEDNESS = {
    enabled: true,
    topK: 3,
    minScore: 0.5,
    maxHops: 2,
...
export function pathFamiliesOf(entries) {
    const set = new Set();
...
export function factKeysOf(entries, activeOnly = true) {
    const set = new Set();
    for (const e of entries) {
        if (e.kind !== 'fact' || !e.factKey)
            continue;
        if (activeOnly && e.status !== 'active')
            continue;
        set.add(e.factKey);
    }
    return set;
...
function collectEdgeCandidates(index, fromId, cfg) {
    const adj = new Map();
    ...
    const out = new Map();
    const queue = [{ id: fromId, score: 0, hops: 0, via: 'self' }];
 ...
 */
export async function relatedTopics(store, fromId, opts) {
    ...
    for (const to of Object.values(index.topics)) {
        if (to.id === fromId || to.status === 'archived')
            continue;
        ...
        for (const t of toTokens)
            if (fromTokens.has(t))
                kw++;
        if (kw > 0) {
            score += Math.min(kw, 3) * cfg.weights.keyword;
            reasons.push(`kw:${kw}`);
        }
        const sj = jaccard(new Set(tokenizeText(fromSummary)), new Set(tokenizeText(toSummary)));
        if (sj > 0) {
            score += cfg.weights.summary * sj;
            reasons.push(`sum:${sj.toFixed(2)}`);
        }
        if (score > 0)
            results.push({ topicId: to.id, score: round1(score), reasons });
    }
    results.sort((a, b) => b.score - a.score);
    return results.filter((r) => r.score >= cfg.minScore).slice(0, cfg.topK);
}
/** 会话→主题匹配（服务端升级客户端 renderMatchView）。 */
export async function matchSessionToTopics(store, features, opts) {
    const cfg = resolveConfig(opts);
    if (!cfg.enabled)
        return [];
    const index = await store.loadIndex();
    const textTokens = tokenizeText(features.texts.join(' '));
    const featureFamilies = new Set(features.paths.map((p) => familyOfPath(p)).filter((x) => x !== null));
    const results = [];
    for (const topic of Object.values(index.topics)) {
        ...
        if (score > 0)
            results.push({ topicId: topic.id, score: round1(score), reasons });
    }
    results.sort((a, b) => b.score - a.score);
    return results.slice(0, cfg.topK);
}
/** 从会话日志尾部提取匹配特征（最近用户输入 + 工具调用）。 */
export function collectSessionFeatures(session, limit = 400) {
    const features = { texts: [], toolNames: [], paths: [], factKeys: [] };
    ...
    for (const event of events) {
        if (event.type === 'user/message') {
            ...
            if (text && features.texts.length < 8)
                features.texts.push(text.slice(0, 400));
        }
        else if (event.type === 'tool/call') {
            const call = event.data;
            if (call?.name && !features.toolNames.includes(call.name))
                features.toolNames.push(call.name);
            for (const raw of extractPaths(call?.arguments ?? '')) {
                if (!seenPaths.has(raw)) {
                    seenPaths.add(raw);
                    features.paths.push(raw);
                }
            }
        }
    }
