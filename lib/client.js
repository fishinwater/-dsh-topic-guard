...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function HeaderTopicButton(props) {
      const listState = typeof props.useSessions === 'function' ? props.useSessions() : null;
    ...
    function FooterTopicsButton(props) {
      const sessions = props.sessions;
      const snap = useSyncExternalStore(panelSubscribe, panelSnapshot);
      // root scope 无 props.sessionId：用 useSessions().current 拿当前会话
      const listState = typeof props.useSessions === 'function' ? props.useSessions() : null;
      ...
      return React.createElement('span', { style: { display: 'inline-flex' } },
        React.createElement('button', { type: 'button', className: 'dsh-tg-hbtn', onClick: function () { openPanel(currentId); }, title: 'Topics' }, wide ? 'Topics' : '◈'),
        renderPanelIfOwner(snap, currentId, sessions)
      );
    }
