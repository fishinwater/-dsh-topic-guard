...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    // ---- 面板状态（模块级单例，按钮与面板共享）----
    const panel = { open: false, sessionId: null, owner: null, view: 'list', tab: 'topics', selectedId: null, list: null, detail: null, match: null, error: null, listeners: new Set(), cached: null };
    // useSyncExternalStore 要求 getSnapshot 返回稳定引用：store 变化时重建快照缓存，
    // 否则每次渲染新对象会触发无限更新（React #185）。
    function panelSnapshot() {
      if (panel.cached === null) {
        panel.cached = { open: panel.open, sessionId: panel.sessionId, owner: panel.owner, view: panel.view, tab: panel.tab, selectedId: panel.selectedId, list: panel.list, detail: panel.detail, match: panel.match, error: panel.error };
      }
      return panel.cached;
    }
    ...
    function renderMatchView(match, onSwitch) {
      const els = [];
      els.push(React.createElement('div', { className: 'dsh-tg-sec', key: 'm1' }, '话题关联（服务端 /t match，非 LLM）'));
      if (!match || match.loading) {
        els.push(React.createElement('div', { className: 'dsh-tg-load', key: 'm2' }, '匹配中…'));
        return els;
      }
      if (match.error) {
        els.push(React.createElement('div', { className: 'dsh-tg-err', key: 'm2' }, '匹配失败（' + match.error + '）'));
        return els;
      }
      ...
      matches.forEach(function (m, i) {
        ...
        els.push(React.createElement('div', { className: 'dsh-tg-match', key: 'm' + i, style: { display: 'flex', alignItems: 'center', gap: 8 } },
          React.createElement('span', { className: 'dsh-tg-id' }, m.topicId),
          React.createElement('span', { style: { fontSize: 11, color: 'var(--dsw-alias-label-caption,inherit)' } }, '得分 ' + m.score + (reasons ? '（' + reasons + '）' : '')),
          React.createElement('button', { type: 'button', className: 'dsh-tg-abtn', style: { marginLeft: 'auto' }, onClick: function () { onSwitch(m.topicId); } }, '设为当前')
        ));
      });
      return els;
    }
    function TopicPanel(props) {
      ...
      // 关联 tab：消费服务端 /t match --json（主题关联度，非 LLM）
      useEffect(function () {
        if (!snap.open || snap.tab !== 'match') { panelPatch({ match: null }); return undefined; }
        if (!remote || !remote.commands) { panelPatch({ match: { error: 'no-remote' } }); return undefined; }
        let alive = true;
        panelPatch({ match: { loading: true } });
        runCommandText(sessionId, remote, '/t match --json').then(function (res) {
          if (!alive) return;
          if (!res || res.kind !== 'success') { panelPatch({ match: { error: 'match-failed' } }); return; }
          let data = null;
          try { data = JSON.parse(res.text); } catch (e) { data = null; }
          panelPatch({ match: data ? { loading: false, data: data } : { error: 'bad-match' } });
        });
        return function () { alive = false; };
      }, [snap.open, snap.tab, sessionId]);
      useEffect(function () {
      ...
      if (snap.tab === 'context') {
        ...
        renderMatchView(snap.match, switchTo).forEach(function (el) { children.push(el); });
