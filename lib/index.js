...
import { relatedTopics, DEFAULT_RELATEDNESS } from "./manager/relatedness.js";
...
export class TopicGuard extends Service {
    ...
    constructor(ctx, config = {}) {
        ...
        const rel = (config.relatedness ?? {});
        const relWeights = (rel.weights ?? {});
        this.config = {
        ...
        this.router = new TopicRouter(this.store, ctx.logger ?? {}, {
            ...
            onFactsChanged: (topicId) => {
                try {
                    this.ctx.emit('topic-guard/facts-changed', { topicId });
                }
                catch { /* best-effort */ }
            },
        });
