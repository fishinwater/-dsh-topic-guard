...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function runCommandText(sessionId, remote, line) {
      ...
      return remote.commands.execute(sessionId, line, []).then(function (exec) {
    ...
    // ---- 入口按钮：会话标题旁 + 侧栏底部 ----
    function renderPanelIfOwner(snap, sessionId, remote) {
    ...
    function FooterTopicsButton(props) {
      ...
      return React.createElement('span', { style: { display: 'inline-flex' } },
        ...
        renderPanelIfOwner(snap, currentId, remote)
      );
    }
