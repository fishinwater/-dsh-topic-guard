...
export class TopicGuard extends Service {
  ...
  constructor(ctx: any, config: Record<string, unknown> = {}) {
    ...
    this.router = new TopicRouter(this.store, ctx.logger ?? {}, {
      relatedness: this.config.relatedness,
      // 事实变更（/t fact add）→ 广播事件，spill 输出池订阅后立即维护（冲突替换 + 落盘）
      onFactsChanged: (topicId: string) => {
        try { (this.ctx as any).emit('topic-guard/facts-changed', { topicId }) } catch { /* best-effort */ }
      },
    });
   ...
   */
  private attachTopicContext(agent: any): void {
    try {
      const sessionId = agent?.session?.id as string | undefined;
      if (!sessionId || typeof agent.ctx?.inject !== 'function') return;
      agent.ctx.inject(['systemPrompt'], (sub: any) => {
   ...
   */
  private registerTopicInjection(): void {
    const ctx = this.ctx as Context;
    try {
      const roots = typeof ctx.agents?.roots === 'function' ? ctx.agents.roots() : [];
      for (const agent of roots) this.attachTopicContext(agent);
    } catch (error) {
    ...
    ctx.on('agent/disposed', ({ agent }: { agent: any }) => {
      const sessionId = agent?.session?.id as string | undefined;
      if (sessionId) this.topicCache.delete(sessionId);
    });
  }
